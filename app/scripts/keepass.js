...
function getKeys(e){
  ...
  btn.disabled = true;
  btn.value = 'Please wait...';
  secretin.loginUser(username, password, document.getElementById('totpToken').value).then(function(){
    btn.disabled = false;
    btn.value = 'Get keys';
    afterLogin();
  }, function(err){
    if(err === 'Need TOTP token'){
      document.getElementById('totp').style.display = 'none';
      setTimeout(function(){document.getElementById('totpToken').focus();},0);
      document.getElementById('totpTokenPopup').style.display = '';
      document.getElementById('submitTotp').addEventListener('click', function(){
        document.getElementById('totpTokenPopup').style.display = 'none';
        getKeys(e);
      })
    }
    else{
...
document.getElementById('createFolder').addEventListener('click', function(e){
  ...
  createFolderBtn.addEventListener('click', function(e){
    secretin.addFolder(title.value).then(function(){
...
document.getElementById('addSecretPopup').addEventListener('click', function(e){
  ...
  addSecretBtn.addEventListener('click', function(e){
    secretin.addSecret(title.value, secret).then(function(){
      secret.destroy();
...
function showSecretPopup(e){
  ...
  secretin.getSecret(hashedTitle).then(function(secretObject){
    var secret = new Secret('secret', secretObject);
    var secretContent = document.getElementById('secretContent');
    ...
    editSecretBtn.addEventListener('click', function(e){
      if(e.target.value === 'Edit Secret'){
        ...
        secretin.editSecret(hashedTitle, secret).then(function(){
...
window.addEventListener('blur', function() {
  timeout = setTimeout(function() { destroyUser(secretin.currentUser); secretin.currentUser = {}; }, 10 * 60 * 1000);
});
...
document.getElementById('share').addEventListener('click', function(e){
  ...
  secretin.shareSecret(hashedTitle, friendName, type, rights).then(function(users){
    getSecretList();
...
document.getElementById('refresh').addEventListener('click', function(e){
  ...
  secretin.refreshUser().then(function(){
    getSecretList();
  }, function(err){
    alert(err);
    throw(err);
...
document.getElementById('getDb').addEventListener('click', function(e) {
  if(typeof secretin.currentUser !== 'undefined' && typeof secretin.currentUser.username !== 'undefined'){
    secretin.api.getDb(secretin.currentUser).then(function(db){
...
document.getElementById('totpToken').addEventListener('keypress', function(e){
  if(e.keyCode === 13){
    document.getElementById('submitTotp').click();
  }
});
