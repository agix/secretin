...
function encryptAESCBC256(secret){
  return new Promise((resolve, reject) => {
    crypto.subtle.generateKey({name: 'AES-CBC', length: 256}, true, ['encrypt']).then((key) => {
      var iv = new Uint8Array(16);
      crypto.getRandomValues(iv);
      var algorithm = {name: 'AES-CBC', iv: iv};
      crypto.subtle.encrypt(algorithm, key, asciiToUint8Array(secret)).then((encryptedSecret) => {
